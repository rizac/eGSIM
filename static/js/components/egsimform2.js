var EGSIMFORM = Vue.component('egsimform', {
  ...
  data: function () {
      return {
          closeButtonVisible: false, /* wheather or not to show close button */
          visible: true,
          fielderrors: {}
      }
  },
  template: `<form :name='url' novalidate v-on:submit.prevent='submitForm'
                v-show="visible">

                <div v-show='closeButtonVisible' class='text-right'>
  ...
  methods: {
      showCloseButton: function(value){
          this.$set(this, 'closeButtonVisible', value);
      },
      ...
      submitForm(){
          ...
          var [data, error] = this.parseForm();
          if(error){
              this.eventbus.$emit('error', error);
          }else{
              this.eventbus.$emit('postrequest', url, this.data, {});
          }
      },
      ...
      parseForm: function(){
          ...
          */
          ...
          for (var elm of this.formElements(form)){
