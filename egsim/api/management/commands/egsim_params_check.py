"""
Simple script to check if there are is any Gsim parameter in OpenQuake that is
unknown and should be implemented in `gsim_params.yaml`.
Ideally, this script should be run after an OpenQuake upgrade when `egsim_init`
is not a full
update of the eGSIM database before
modifying the entire db.

Usage:
```
export DJANGO_SETTINGS_MODULE="..."; python manage.py egsim_params_check
```
