...
class Esm2018:
    ...
    def parse(file) -> pd.DataFrame:
        ...
        :param file: str, path object or file-like object denoting the input to be
            read and parsed
        """
        dfr = read_flatfile(file, csv_sep=Esm2018.sep, rename=Esm2018.rename,
                            dtypes=Esm2018.extra_dtype,
                            defaults={}, usecols=Esm2018.usecols)

        return Esm2018.post_process(dfr)
    ...
    def usecols(colname):
        ...
        if colname.startswith('rotD'):
            return colname in Esm2018.rename  # global var
    ...
    def post_process(flatfile: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in (c for c in rename if c.startswith('rotD50_')):
...
class KiknetKnet:
    ...
    def parse(file) -> pd.DataFrame:
        ...
        :param file: str, path object or file-like object denoting the input to be
            read and parsed
        """
        ff2: pd.DataFrame = pd.read_hdf(file)  # noqa
        # fix for bad evt_time
        ff2['evt_time'] = pd.to_datetime(
            ff2['evt_time'].str.replace("/", "-").str.replace(",", " ")
        )

        # remove columns:
        assert sorted([c for c in ff2.columns if 'object' in str(ff2[c].dtype)]) == \
               ['Address', 'style_of_faulting']
        assert ff2['rup_top_depth'].isna().all()  # FIXME: as Karina. for now, remove
        assert len(pd.unique(ff2['evt_country'])) == 1  # ['JPN']
        assert len(pd.unique(ff2['sta_nation_code'])) == 1  # ['JPN']
        # remove object columns and also other two that are not default flatfile
        # fields. We should save around 50-60 Mb per dataframe
        ff2.drop(columns=['Address', 'style_of_faulting', 'highpass_h1', 'lowpass_h1',
                          'duration_5_95_components',
                          'rup_top_depth', 'evt_country', 'sta_nation_code',
                          # remove some SA (checked visually):
                          'SA(0.022)', 'SA(0.029)', 'SA(0.032)', 'SA(0.036)',
                          'SA(0.042)', 'SA(0.044)', 'SA(0.046)', 'SA(0.048)',
                          'SA(0.067)', 'SA(0.133)', 'SA(0.290)', 'SA(0.667)'],
                 inplace=True)
        # To check the produced dataframe: uncomment the following lines:
        # ff2.to_hdf(file + '.cleaned.tmp.hdf', format='table', key='cleaned_hdf')
        # dfr = read_flatfile(file + '.cleaned.tmp.hdf')
        return KiknetKnet.post_process(ff2)

    @staticmethod
    def post_process(dataframe):
        return dataframe
